CREATE VIEW candidats_competences AS
  SELECT c.identifiant, de.de_fr, de.de_en
  FROM domaines_etudes de
  LEFT JOIN formations f ON de.id_de = f.domaine_etude
  JOIN suivre_formation sf ON f.id_formation = sf.id_formation
  JOIN candidats c ON sf.id_candidat = c.id_candidat
;

CREATE VIEW individus_candidats AS
  SELECT i.nom, i.prenom, i.mail, c.telephone, c.telephone_type, c.url_web, c.type_web
  FROM individus i
